...
import torch
import numpy as np
from model_GC import FourierDeepONet
from data_GC import dataset
...
def main():
    ...
    net = FourierDeepONet(num_parameter=X_train[1].shape[1], width=32, modes1=50, modes2=30, regularization=["l2", 1e-6])
    ...
    losshistory, train_state = model.train(iterations=100000, batch_size=32, display_every=1000, callbacks=[checker])
